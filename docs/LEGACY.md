...
XC1 (Wii) and XCX use big endian. XC2/DE files are little-endian.
...
| Magic (`b"BDAT"`)                                        | u32                                 |                                                         |
| Flags (see below)                                        | u8                                  |                                                         |
| ??? (0)                                                  | u8                                  | Not part of Flags, as it doesn't respect the byte order |
...
| **<u>The next fields are absent in XC1 (Wii) BDATs</u>** | <--                                 | <--                                                     |
| Column node table offset                                 | u16                                 |                                                         |
| Number of column nodes                                   | u16                                 |                                                         |
| Padding                                                  | total header size = 64 (32 for Wii) |                                                         |

### Known table flags

| Bit | Mask | Notes |
| --- | ---- | ----- |
| 0 | 0x1 | *Unknown*. Most likely 1 for Big Endian tables (only XC1 and XCX have it set) |
...
In XC1 (Wii) BDATs, these nodes are used instead of column/flag names in the name table.
...
The name table is a bit different in XC1 (Wii), as it stores column nodes as well. The table name is still
...
| 8 | Float | 4 | IEEE-754 floating point (1/2/DE), `20.12` fixed-point (`f = raw / 4096.0`) (XCX) |
